 ...
 *
 * Copyright (C) 1999 Tom Tromeyg_utf8_get_char
...
#include <glib/gunichartables.h>
#include <glib/gmirroringtable.h>
#include <glib/gscripttable.h>
	      ...
	      gboolean     remove_dot)
{
  ...
  while (*p)
    {
      gunichar c = *p; // g_utf8_get_char (p);
...
has_more_above (const gchar *str)
{
  ...
  while (*p)
    {
      combining_class = g_unichar_combining_class (*p); // g_utf8_get_char (p));
  ...
  while ((max_len < 0 || p < str + max_len) && *p)
      ...
      else if (c == 0x03A3)	/* GREEK CAPITAL LETTER SIGMA */
	{
	  if ((max_len < 0 || p < str + max_len) && *p)
	    {
	      gunichar next_c = *p; // g_utf8_get_char (p);
		 ...
		 gssize       len)
{
  ...
  while ((len < 0 || p < str + len) && *p)
    {
      gunichar ch = *p; //*p; // g_utf8_get_char (p);
